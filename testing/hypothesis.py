...
class hypothesis():
    ...
    def predict(self, inputs):
        for x in range(len(inputs)):
            inputs[x] = (inputs[x] - self.normalization_values[x][2])/(self.normalization_values[x][0] - self.normalization_values[x][1])
    ...
    def process_data(self, df):
        ...
        for column in range(len(self.X[0])):
            # Get max, get min, get average
            max = self.X[:, column].max()
            min = self.X[:, column].min()
            mean = self.X[:, column].mean()
            normals = [max, min, mean]
            self.normalization_values.append(normals)

        for column in range(len(self.X[0])):
            for x in range(len(self.X[:, column])):
                self.X[x, column] = (self.X[x, column] - self.normalization_values[column][2])/(self.normalization_values[column][0] - self.normalization_values[column][1])
